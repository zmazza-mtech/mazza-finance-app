...
- Decision 9 measured, then resolved: the unoptimized forecast averaged
  ≈ 106ms; after the Phase 1 performance pass (#67) the same load runs at
  **≈ 2ms avg**, inside the 10ms free-tier budget, and the bench now asserts
  the budget rather than a regression ceiling. Workers Paid is not needed for
  CPU. Measurement caveat baked into the bench: workerd pins
...
| 9 | Forecast CPU | **Measured 2026-08-17** under workerd (`worker/tests/forecast-cpu.spec.ts`): the unoptimized pipeline averages **~106ms** on a heavy 226-day load — ~10× over the free-tier 10ms budget | The cost is quadratic hot spots, not decimal.js itself: the per-day `filter` over all transactions in `computeForecast` and per-series Decimal-heavy matching in `reconcileInstances`. **Phase 1 therefore includes a forecast performance pass** (group transactions by date, index actuals by date in reconciliation, reuse Decimal instances) expected to yield 10–20×; Workers Paid ($5/mo, 30s CPU) stays as the fallback if the pass can't reach budget. The pass helps phone latency on any tier, so it is not tier-contingent work. **Resolved by #67, 2026-08-17:** the pass landed both indexes and measured **32.33ms → 2.03ms avg** (min 2, max 3) on the same load, a 16× gain, inside the budget with margin. The bench assertion is now the 10ms budget itself. Output is unchanged, pinned by a frozen 226-day golden fixture rather than by inspection. **The Paid fallback is not exercised: the free tier stands.** |
